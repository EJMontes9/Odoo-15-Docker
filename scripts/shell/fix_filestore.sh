#!/bin/bash
# Script to fix the filestore directory structure in the Odoo container
# This script will:
...
echo "Fixing filestore directory structure in Odoo container..."
...
echo "Filestore directory structure fixed. Restarting Odoo container..."

# Restart the Odoo container to apply the changes
docker restart odoo-app

echo "Odoo container restarted. The fix has been applied."
echo "You can verify the fix by checking the logs: docker logs odoo-app"